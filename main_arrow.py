...
def identify_colors(mask,image, color_name):
    ...
    if contours:
        ...
        if 0.8 <= aspect_ratio <= 1.3:  # 接近正方形
            ...
            # return color_name, largest_area
            if color_name == "Green":
                if largest_area > 150:  # 過濾雜訊

                    results = model5(image)
                    ArrowSequence = ["L", "^", "R"] #表示三種class的方向
                    for result in results:
                        if hasattr(result, 'probs') and result.probs is not None:
                            ...
                            print(f"Predicted Class: {ans}, Confidence: {confidence:.2f}")
                            
                            cv2.imshow("Green Pre", mask)
                            cv2.waitKey(0)
                            cv2.destroyWindow("Green Pre")
                            return ans, largest_area
                        else:
                            print("No classification results found.")

                    # # cv2.imshow("Green Pre",  model5.predict(image))
                    # result = model5(image)
                    # result[0].show()
                    return "*", largest_area
    return None, 0
...
# model5 = YOLO("bestSingleArrowE4.pt")
model5 = YOLO("bestSingleArrowTrain16.pt")
